...
export class DefinitionsFactory {
  static createForClass(target: Function): mongoose.SchemaDefinition {
    if (!target) {
      throw new Error(
  ...
  private static inspectTypeDefinition(
    ...
    if (typeof optionsOrType === 'function') {
      ...
      const schemaDefinition = this.createForClass(optionsOrType as Function);
      const schemaMetadata = TypeMetadataStorage.getSchemaMetadataByTarget(
        optionsOrType as Function
      );
