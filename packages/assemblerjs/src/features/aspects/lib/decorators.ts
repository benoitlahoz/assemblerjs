...
/**
 * Checks if a class is decorated with @Aspect
 * @param target The class to check
 * @returns True if the class is an aspect
 */
export function isAspect(target: any): boolean {
  try {
    const definition = getDefinition(target);
    return definition?.metadata?.isAspect === true;
  } catch {
    return false;
  }
}
...
/**
 * Decorator to define a class as an Aspect.
 * An Aspect is a specialized Assemblage that:
 ...
 * ```
 */
export function Aspect(definition?: Omit<AssemblageDefinition, 'inject' | 'use'>) {
  return function <T extends new (...args: any[]) => any>(target: T) {
    // Validate that definition doesn't have inject or use
    if (definition) {
      if ('inject' in definition || 'use' in definition) {
        throw new Error(
          `@Aspect on class ${target.name} cannot have inject or use properties. ` +
    ...
    const existingAdvices = getCustomMetadata(ReflectValue.AspectAdvices, target) || [];

    // An Aspect is a specialized Assemblage (singleton by default, no inject/use)
    const aspectDefinition: AssemblageDefinition = {
      ...
      inject: [],
      use: [],
      // Add metadata to identify this as an aspect and preserve advices
      metadata: {
        ...definition?.metadata,
        isAspect: true,
